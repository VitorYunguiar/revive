...
export default function SyncScreen() {
  ...
  return <Screen>
    ...
    {items.map((item) => <Card key={item.id}>
      <Text style={textStyles.heading}>{labels[item.type]}</Text>
      ...
      {item.status === 'failed' ? <AppButton title="Tentar novamente" disabled={busy} onPress={() => user && void run(async () => { await retryUserMutation(user.id, item.id); await syncPendingMutations(user.id); })} /> : null}
